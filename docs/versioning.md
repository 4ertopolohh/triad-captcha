...
Both packages share one Semantic Version. A normal shared
...
```bash
git tag -a v0.2.0 -m "TriadCAPTCHA 0.2.0"
```
...
```bash
git subtree split --prefix=packages/react -b release/react-0.2.0
git tag -a react-v0.2.0 release/react-0.2.0 -m "@triadcaptcha/react 0.2.0"
```
